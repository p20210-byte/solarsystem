...
planet_data = {
    ...
    '목성': {
        ...
        '대표적인 위성': '이오', '유로파', '가니메데', '칼리스토'
    }

        satelliteJ = st.selectbox('위성을 선택해주세요', ['이오', '유로파', '가니메데', '칼리스토'])
    ...
    '토성': {
        ...
        satelliteS = st.selectbox('위성을 선택해주세요', ['타이탄', '엔셀라두스', '미마스']):
        ...
        satelliteS_data = {
            ...
            '엔살라두스' : {'토성의 E고리에 위치한 위성이다.'
                       '이름은 그리스 신화에서 등장하는 엔켈라두스에서 따왔다.'
                       '지름이 504km 밖에 되지 않아 영국보다 작은 크기를 가지고 있다.'
                       '일찍 형성되어 토성의 고리가 형성될 때 같이 형성되었을 것으로 추측하고 있다.'
            }

            '미마스' : {'표면에 지름의 3분 1에 달하는 지름 130km 정도의 거대한 크레이터가 있다.'
    ...
    '천왕성': {
        ...
        satelliteU = st.selectbox('위성을 선택해주세요', ['세티버스', '티타니아', '미란다']):

        satelliteU_data = {
            '아리엘' : {'천왕성의 제 1위성이다.'
                     '천왕성의 대형 위성 중 두 번째로 천왕성과 가깝다.'
                     '이심률과 궤도 경사각이 매우 작아 누워있는 상태로 공전한다.'
                     '천왕성의 위성 중에서 가장 반사율이 높아 표면 지형과 색이 연관성을 거의 없다.'
            }

            '티타니아' : {'천왕성의 5개의 대형 위성 중 하나로, 천왕성의 위성들 중 가장 크다.'
                      '이심률이 작아 궤도가 원에 가깝고 경사각이 작아 누운 상태로 작용한다.'
                      '탐사선이 발사하고 가는 시간만 13년이 예상되어 티타니에 접근하기 힘들다.'
                      '현재까지 티타니아의 표면은 40% 정도 알려져 있다.'
            }

            '미란다' : {'천왕성의 5대 위성 중에서 가장 작고 천왕성에 가장 가까운 궤도를 돌고 있다.'
                     '코로나 상태이며, 다른 위성과 충돌하면서 분해되었다가 다시 합쳐졌다.'
                     '깊이 20km짜리 계곡이 3개를 가지고 있다.'
                     '티타니아와 마찬가지로 탐사선이 도달하는데 오랜 시간이 걸린다.'       
            }
    },
    ...
    '해왕성': {
        ...
        '대표적인 위성': '트리톤, 나이아드, 탈라사 등'
         ...
         satelliteN = st.selectbox('위성을 선택해주세요', ['트리톤', '네레이', '나이아드']):

         satelliterN_data = {
            '트리톤' : {'해왕성의 제 1위성이다.'
                     '해왕성의 위성 중에서 가장 크기가 크다.'
                     '공전 방향이 행성 공전 방향과 반대인 위성 중 하나이다.'
                     '해왕성과 점점 가까워지고 있으며, 대략 36억 년 후에 해왕성에 의해 파괴될 운명이 예측된다.'
            }

            '네레이드' : {'해왕성의 제 2위성이다.'
                      '현재까지 발견된 위성 중에서 가장 높은 이심률을 가진다.'
                      '본래는 원형 궤도였지만 트리톤의 포획으리 인해 궤도가 틀어졌다.'
            }

            '나이아드' : {'해왕성의 제 3위성이다.'
                     '해왕성과 거리가 가장 가까운 위성이다.'
                      '불규칙한 모양을 가지고 있어 해왕성 해왕성 중심에서 약 48000km 떨어진 궤도에서 공전한다.'
            }
}}
